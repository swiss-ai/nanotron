"""
    python3 tools/check_dataset.py datasets/SlimPajama-6B-b --tokenizer meta-llama/Meta-Llama-3-8B
"""
import argparse
import os
import struct
...
parser.add_argument("--eos", type=str, help="eos token", default="<|end_of_text|>")
...
def load_dataset_bytes(file, doc_ends, bytes_per_value: int = 4):
    """
...
def check_dataset(input_folder: DataFolder, tokenizer: str = "gpt2", eos_token: str = "<|endoftext|>"):
    ...
    for filei, (file_doc_ends, file_token_inputs, file_loss_inputs) in enumerate(
        ...
        for doci, tokens in tqdm(enumerate(file_token_inputs), total=len(file_doc_ends)):
            last_token = struct.unpack("<I", tokens[-4:])[0]
