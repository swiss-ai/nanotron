"""
    python3 tools/count_SFT_tokens.py --hf_dataset Magpie-Align/Magpie-Pro-300K-Filtered --path_to_tokenizer meta-llama/Meta-Llama-3.1-8B-Instruct
"""
import argparse
...
def compute_length(sample, chat_tokenizer, conversation_column_name):
    return {"sample_length": len(chat_tokenizer(sample[conversation_column_name])[0])}


def main(args):
    # Load dataset & tokenizer
    ds = load_dataset(args.hf_dataset, split=args.dataset_split)
    chat_tokenizer = ChatTokenizer(args.path_to_tokenizer)

    ds = ds.map(
        compute_length,
        fn_kwargs={"chat_tokenizer": chat_tokenizer, "conversation_column_name": args.conversation_column_name},
        batched=False,
        num_proc=64,
    )
    print(f"Total number of tokens of the {args.hf_dataset} dataset: {sum(ds['sample_length'])}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SFT Count tokens tool")
    parser.add_argument(
        "--hf_dataset",
        type=str,
        required=True,
        help="Name of the dataset in the HF Hub e.g. Magpie-Align/Magpie-Pro-300K-Filtered",
    )
    parser.add_argument(
        "--path_to_tokenizer",
        type=str,
        required=True,
        help="Path a local folder or a HF hub repo e.g. meta-llama/Meta-Llama-3.1-8B-Instruct",
    )
    parser.add_argument(
        "--dataset_split", type=str, default="train", help="Dataset split to compute the total number of tokens"
    )
    parser.add_argument(
        "--conversation_column_name",
        type=str,
        default="conversations",
        help="Column from the dataset containing a list of dictionaries where each dictionary is a message",
    )
